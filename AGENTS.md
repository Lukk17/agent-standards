...
| GitHub Copilot | [.github/hooks/preflight.json](.github/hooks/preflight.json) | `sessionStart` for the gate text and the task list, `subagentStart` for the subagent text, `userPromptTransformed` for the gate text on every prompt through `copilot/prompt_reminder.py`, `preToolUse` (matcher `bash|powershell|create|edit|apply_patch`\|powershell\|create\|edit`) for the gate, `agentStop` for the formatting checker |
