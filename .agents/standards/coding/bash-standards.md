...
- Validate all required positional or named arguments at script entry; print a usage message and exit 1 if any are missing or invalid. [Confidence: 100%]
- Use `[[ ]]` for conditional tests instead of `[ ]`; the double-bracket form is safer and supports regex matching. [Confidence: 100%]
...
- Use `getopts` for single-character flags; use a manual `case` statement for long options (`--flag`). [Confidence: 100%]
- Always implement `-h` / `--help` that prints usage and exits 0. [Confidence: 100%]
...
- Exit codes 3–125 may be defined per-script for specific error conditions; document them in the header. [Confidence: 100%]
...
- Always create temp files with `mktemp`; never hardcode `/tmp/script-name.tmp`. [Confidence: 100%]
- Always remove temp files in the `cleanup` trap; never rely on manual cleanup at the end of the script. [Confidence: 100%]
...
- Never echo, print, or log secrets. If a secret must be passed to a subprocess, use a file descriptor or environment variable scoped to that subprocess. [Confidence: 100%]
- Read secrets from files or environment variables; never accept them as positional command-line arguments (they appear in `ps` output). [Confidence: 100%]
...
- Run **ShellCheck** on every script in CI with zero warnings/errors policy. Suppress individual rules only with an inline `# shellcheck disable=SCxxxx` comment that includes a justification. [Confidence: 100%]
