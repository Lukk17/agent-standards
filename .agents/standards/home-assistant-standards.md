...
- Use the pattern `domain.location_device_property` for entity IDs (e.g., `sensor.living_room_temperature`, `light.bedroom_ceiling`). [Confidence: 100%]
- Never abbreviate entity names; prefer clarity over brevity (`binary_sensor.front_door_contact` not `binary_sensor.fd_c`).
- Use lowercase with underscores only; no spaces, hyphens, or capital letters in entity IDs. [Confidence: 100%]
...
- Store all long-lived access tokens, API keys, and passwords in `secrets.yaml`; reference them in configuration with `!secret key_name`. [Confidence: 100%]
...
- Use the **Home Assistant Backup** integration (built-in since HA 2024.11) or the **Google Drive Backup** add-on to take daily automated backups. [Confidence: 100%]
...
- Prefer **native HA integrations** (configured via the UI at Settings → Devices & Services) over YAML-configured custom integrations. [Confidence: 100%]
...
- Extract any automation logic used in more than two automations into a **Script** or **Blueprint**. [Confidence: 100%]
...
- Use Developer Tools → **Automation Trace** after every automation change to verify the execution path and confirm conditions behaved as expected. [Confidence: 100%]
...
- Implement notification routing as a HA Script with a `priority` input parameter; call it from all automations instead of hardcoding `notify.*` targets. [Confidence: 100%]
...
- Document the minimum supported Home Assistant version for any automation or integration that uses features introduced in a specific release (e.g., `# Requires HA 2024.10+`). [Confidence: 100%]
