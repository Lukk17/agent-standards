...
*   If developing on a Chromebook: Turn on Linux support in the ChromeOS settings. Ensure the Linux container is fully
    updated using the `apt-get` commands above before proceeding.

3. IDE Configuration
Install Visual Studio Code, Android Studio, or an IntelliJ-based IDE. Install the official Dart and Flutter
extensions/plugins to enable language server features and debugging capabilities.
...
*   If errors exist under the "Linux toolchain" section: Review the missing dependencies, install the flagged packages,
    and re-run `flutter doctor -v`. Repeat until the Linux toolchain section passes.
