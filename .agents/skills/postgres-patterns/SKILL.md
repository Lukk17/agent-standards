...
- JPA and Hibernate entity mapping and fetch strategy: use `jpa-patterns`.
- MongoDB modelling and indexing: use `mongodb-schema-design` and `mongodb-query-optimizer`.
...
- `jpa-patterns` when the queries are generated by Hibernate rather than written by hand.
