...
*   If retaining user session state (navigation, scroll positions): Implement Flutter's built-in state restoration to
    sync the Element tree with the engine.
...
Yield local data immediately for fast UI rendering, then fetch remote data, update the local cache, and yield the fresh
data.
...
*   If strict server synchronization is required (Online-only): Attempt the API call first. Only update the local
    database if the API call succeeds.
*   If offline availability is prioritized (Offline-first): Write to the local database immediately. Attempt the API
    call. If the API call fails, flag the local record for background synchronization.

#### Background Synchronization
Add a `synchronized` boolean flag to your data models. Keep the model immutable (Freezed): never mutate the flag in
place. Produce a new instance with `copyWith(synchronized: true)` and write that back. Run a periodic background task
(e.g., via `workmanager` or a `Timer`) to push unsynchronized local changes to the server.
...
*   Custom ImageProviders: If implementing a custom `ImageProvider`, override `createStream()` and
    `resolveStreamForKey()` instead of the deprecated `resolve()` method.
*   Cache Sizing: The `ImageCache.maxByteSize` no longer automatically expands for large images. If loading images
    larger than the default cache size, manually increase `ImageCache.maxByteSize` or subclass `ImageCache` to implement
    custom eviction logic.

#### Scroll Caching
When configuring caching for scrollable widgets (`ListView`, `GridView`, `Viewport`), use the `scrollCacheExtent`
property with a `ScrollCacheExtent` object. Do not use the deprecated `cacheExtent` and `cacheExtentStyle` properties.
...
*   Exception: You may override `operator ==` only on leaf widgets (no children) where comparing properties is
    significantly faster than rebuilding, and the properties rarely change.
...
To eliminate the non-trivial warm-up time of a `FlutterEngine` when adding Flutter to an existing Android app, pre-warm
and cache the engine.
...
```
*Note: You cannot set an initial route via the Activity/Fragment builder when using a cached engine. Set the initial
route on the engine's navigation channel before executing the Dart entrypoint.*
