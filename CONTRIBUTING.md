...
- **Generator output in sync.** If you edit any canonical subagent under `subagents/`, re-run
  `python tools/gen-subagents.py` and commit the regenerated copies under `.claude/agents/` and `.opencode/agents/`.
...
(`name:`, `description:`). Pattern-match an existing skill (e.g. [`coding-standards`](.agents/skills/coding-standards/SKILL.md))
for shape.
...
Add a frontmatter-headed Markdown file under `subagents/<name>.md`. Run `python tools/gen-subagents.py` to emit
